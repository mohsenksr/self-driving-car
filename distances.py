...
def is_left_empty():
    ...
    pulse_end = 0
    # Set trigger to HIGH
    GPIO.output(LEFT_SENSOR_TRIGGER_PIN, GPIO.HIGH)
    time.sleep(0.00001)
    GPIO.output(LEFT_SENSOR_TRIGGER_PIN, GPIO.LOW)
    # Wait for echo to go high
    while GPIO.input(LEFT_SENSOR_ECHO_PIN) == GPIO.LOW:
        pulse_start = time.time()
    # Wait for echo to go low
    while GPIO.input(LEFT_SENSOR_ECHO_PIN) == GPIO.HIGH:
        pulse_end = time.time()
    # Calculate distance
...
def is_right_empty():
    ...
    pulse_end = 0
    # Set trigger to HIGH
    GPIO.output(RIGHT_SENSOR_TRIGGER_PIN, GPIO.HIGH)
    time.sleep(0.00001)
    GPIO.output(RIGHT_SENSOR_TRIGGER_PIN, GPIO.LOW)
    # Wait for echo to go high
    while GPIO.input(RIGHT_SENSOR_ECHO_PIN) == GPIO.LOW:
        pulse_start = time.time()
    # Wait for echo to go low
    while GPIO.input(RIGHT_SENSOR_ECHO_PIN) == GPIO.HIGH:
        pulse_end = time.time()
    # Calculate distance
